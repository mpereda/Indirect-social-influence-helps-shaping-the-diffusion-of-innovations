...
def set_payoff(group: Group):
    ...
    for p in players:
        ...
        if c["#e53950"] < c["#ffa460"]:
            p.botcolor = "#ffa460"
        elif c["#e53950"] > c["#ffa460"]:
            p.botcolor = "#e53950"
        else:
            p.botcolor = np.random.choice(["#e53950", "#ffa460"])
    ...
    def before_next_page(player, timeout_happened):
        if timeout_happened:
            ...
            if (player.round_number == 1):
                ...
                player.action = random.choice([str(random.choice(['#e53950', '#ffa460'])),str(prev_player.botcolor)])
        else:
            player.bot=False
